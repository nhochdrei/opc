...
func (ai *AutomationItems) readVariantFromOpc(opcitem *ole.IDispatch, vt ole.VT) (Item, error) {
	...
	if vt&ole.VT_ARRAY != 0 {
		v.VT = vt
		arr := v.ToArray()
		logger.Println("RETURN ARRAY", v.VT&^ole.VT_ARRAY, arr.Array.ElementsSize, "DIM", arr.Array.Dimensions, "BOUNDS", arr.Array.Bounds, "FEATURES", arr.Array.FeaturesFlag, "ELEMTYPE", arr.GetVarType())
		// _, err = arr.GetData()
